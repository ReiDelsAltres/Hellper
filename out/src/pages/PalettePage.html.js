...
let PalettePage = class PalettePage extends Page {
    paletteGallery;
    themeGallery;
    paletteStyleElements = [];
    previewHandles = new WeakMap();
    ...
    async loadAllPaletteStyles() {
        const names = new Set(COLOR_PALETTES.map(p => p.name));
        ...
        const loadPromises = Array.from(names).map(async (name) => {
            if (document.querySelector(`style[data-palette-preview="${name}"]`))
                return;
            try {
    ...
    renderThemes() {
        ...
        for (const theme of APP_THEMES) {
            ...
            try {
                this.createThemePreview(item, theme);
            }
            catch (e) { /* ignore preview failures */ }
        }
    }
    createThemePreview(item, theme) {
    ...
    createDyBlazorPreview(container) {
        const g = document.createElement('div');
        g.className = 'preview-gradient animated';
