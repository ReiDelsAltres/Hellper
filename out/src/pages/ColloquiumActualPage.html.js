...
let ColloquiumActualPage = class ColloquiumActualPage extends Page {
    ...
    showScoreResult(scoreEl, lengthEl, breakdownEl, result) {
        ...
        if (breakdownEl && result.breakdown) {
            ...
            const bonusParts = [];
            if (typeof b.similarityBonusPercent === 'number' && b.similarityBonusPercent > 0)
                bonusParts.push(`Семантика +${b.similarityBonusPercent}%`);
            if (typeof b.coverageBonusPercent === 'number' && b.coverageBonusPercent > 0)
                bonusParts.push(`Покрытие +${b.coverageBonusPercent}%`);
            if (typeof b.keywordBonusPercent === 'number' && b.keywordBonusPercent > 0)
                bonusParts.push(`Ключевые слова +${b.keywordBonusPercent}%`);
    ...
    async confirmBilet(biletIdx) {
        ...
        for (let qIdx = 0; qIdx < bilet.questions.length; qIdx++) {
            ...
            const emptyBd = { similarity: 0, coverage: 0, keyword: 0, hasKeywords: false, confidenceModifier: 1, criticalError: false, augmented: false, numericMismatch: false, similarityBonusPercent: 0, coverageBonusPercent: 0, keywordBonusPercent: 0, bonusMultiplier: 1, similarityWeight: 0, coverageWeight: 0, keywordWeight: 0 };
    ...
    async confirmFinish() {
        ...
        if (this.isBiletMode) {
            for (let bidx = 0; bidx < this.bilets.length; bidx++) {
                ...
                for (let qidx = 0; qidx < bilet.questions.length; qidx++) {
                    const q = bilet.questions[qidx];
                    const eBd = { similarity: 0, coverage: 0, keyword: 0, hasKeywords: false, confidenceModifier: 1, criticalError: false, augmented: false, numericMismatch: false, similarityBonusPercent: 0, coverageBonusPercent: 0, keywordBonusPercent: 0, bonusMultiplier: 1, similarityWeight: 0, coverageWeight: 0, keywordWeight: 0 };
            ...
            for (let qidx = 0; qidx < this.questions.length; qidx++) {
                ...
                const eBd2 = { similarity: 0, coverage: 0, keyword: 0, hasKeywords: false, confidenceModifier: 1, criticalError: false, augmented: false, numericMismatch: false, similarityBonusPercent: 0, coverageBonusPercent: 0, keywordBonusPercent: 0, bonusMultiplier: 1, similarityWeight: 0, coverageWeight: 0, keywordWeight: 0 };
