...
export const PREVIEW_RENDERERS: Record<string, (container: HTMLElement) => PreviewCleanup> = {
    ...
    Neon(container: HTMLElement) {
        ...
        const particles: { el: HTMLDivElement; x: number; y: number; vx: number; vy: number; radius: number; color: string; intensity: number; }[] = [];
        ...
        return () => {
            if (raf) cancelAnimationFrame(raf);
            particles.forEach(p => { try { p.el.remove(); } catch { } });
            try { overlay.remove(); } catch { }
        };
    },
};
