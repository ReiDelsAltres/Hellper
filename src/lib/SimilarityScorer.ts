...
const ss = (text: string) => new SemanticString(text);

interface SemanticScoreInputs {
    userSS: SemanticString;
    answerSS: SemanticString[];
    keywordSS: SemanticString[];
    questionTitleSS: SemanticString | null;
}

// ===== Stage 1: Length Check =====
function checkLength(
    text: string, answerSS: SemanticString[], idealSize?: number,
...
async function directSimilarity(
    userSS: SemanticString,
    answerSS: SemanticString[],
): Promise<number> {
...
async function sentenceCoverage(
    userSS: SemanticString,
    answerSS: SemanticString[],
): Promise<number> {
    const userText = userSS.raw;
    if (answerSS.length === 0) return 0;
    ...
    for (let ai = 0; ai < answerSS.length; ai++) {
        ...
        if (refSents.length === 0) refSents.push(refSS.raw);
...
async function keywordScore(
    userSS: SemanticString,
    keywordSS: SemanticString[],
): Promise<number> {
    const userText = userSS.raw;
    if (keywordSS.length === 0) return 0;
    ...
    for (let i = 0; i < keywordSS.length; i++) {
        ...
        if (kwNums.length > 0) {
            ...
            for (const kn of kwNums) {
                if (userNums.some(un => numberEq(un, kn))) matchedNums++;
            }
...
async function computeSemanticLogicScore(
    input: SemanticScoreInputs,
): Promise<{ score: number; similarity: number; coverage: number; kw: number; augmented?: boolean; effectiveText?: string; numericMismatch?: boolean; negationContradiction?: boolean }> {
    const { userSS, answerSS, keywordSS } = input;
    const userText = userSS.raw;

    // Short-answer overrides: detect numeric answers, acronyms, or exact-token subset
    function computeShortAnswerOverride(
        uSS: SemanticString,
        answersListSS: SemanticString[],
        kwsSS: SemanticString[],
        questionTitleSS: SemanticString | null,
    ): { similarity?: number; coverage?: number; kw?: number; reason?: string; augment?: string } | null {
        ...
        const calcOverlap = (group: { tokens: string[] }): number => {
            let hits = 0;
            for (const t of group.tokens) if (userTokenSet.has(t)) hits++;
    ...
    const shortOverride = computeShortAnswerOverride(
        userSS,
        answerSS,
        keywordSS,
        input.questionTitleSS,
    );
...
async function computeConfidenceModifierUnifiedSafe(
    ...
    const userText = userSS.raw;
    if (!answerSS.length || !userText.trim()) return 1.0;
...
export default class SimilarityScorer {
    ...
    static async score(
        ...
        const semanticInput: SemanticScoreInputs = {
            userSS: ss(trimmed),
        ...
        if (semanticInput.answerSS.length === 0) {
            const kw = await keywordScore(
                semanticInput.userSS,
                semanticInput.keywordSS,
            );
            return {
        ...
        let criticalError = !!(sem as any).negationContradiction;
        ...
        let confidenceModifier = await computeConfidenceModifierUnifiedSafe(
            cmSS,
            semanticInput.answerSS,
            semanticInput.keywordSS,
        );
        ...
        const perBonus = 0.2;
        const hasNumericMismatch = !!(sem as any).numericMismatch;
