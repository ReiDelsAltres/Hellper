...
import { Semestr, ExamFile, ExamQuestion } from "../frac/Testing.js";
...
export type HeaderInput =
    Pick<QuestionHeader, "subjectFile" | "questionId"> & Partial<QuestionHeader>;
...
export default class QuestionLibrary {
    ...
    private static _entries: Map<string, QuestionEntry> | null = null;

    // ── Catalog: subjectFile → { semester, subjectName } ──
    private static _catalog: Map<string, { semester: string; subjectName: string }> | null = null;
    private static _catalogPromise: Promise<void> | null = null;

    // ── Content cache: subjectFile → (questionId → render-ready model) ──
    private static _content: Map<string, Map<string, QuestionModel>> = new Map();
    private static _contentPromises: Map<string, Promise<Map<string, QuestionModel>>> = new Map();

    // ──────────────────────────────────────────────────────────────────────────
    // Identity
    // ──────────────────────────────────────────────────────────────────────────

    private static key(subjectFile: string, questionId: string | number): string {
        return subjectFile + "::" + String(questionId);
    }

    /** Map a {@link QuestionComponent} status to a recordable result (`unanswered` → null). */
    public static statusToResult(status: string): AnswerResult | null {
        switch (status) {
     ...
     */
    public static ensureCatalog(): Promise<void> {
        if (this._catalogPromise) return this._catalogPromise;
        this._catalogPromise = (async () => {
            this._catalog = new Map();
            try {
                const semesters = await Fetcher.fetchJSON(CATALOG_URL) as Semestr[];
    ...
    public static getSemester(subjectFile: string): string {
        return this._catalog?.get(subjectFile)?.semester ?? "";
    }

    /** Best-known display name for a subject file, falling back to the file itself. */
    public static getSubjectName(subjectFile: string, fallback?: string): string {
    ...
    private static load(): Map<string, QuestionEntry> {
        if (this._entries) return this._entries;
        const map = new Map<string, QuestionEntry>();
        try {
            const raw = localStorage.getItem(STORAGE_KEY);
            if (raw) {
                const parsed = JSON.parse(raw) as Record<string, QuestionEntry>;
                for (const [k, v] of Object.entries(parsed)) {
                    if (v && v.header && v.statistics && v.meta) map.set(k, v);
                }
            }
        } catch (e) {
    ...
    private static persist(): void {
        ...
        try {
            const obj: Record<string, QuestionEntry> = {};
            for (const [k, v] of this._entries) obj[k] = v;
    ...
    private static resolveHeader(input: HeaderInput): QuestionHeader {
        return {
    ...
    private static ensureEntry(input: HeaderInput): QuestionEntry {
        ...
        if (!entry) {
            ...
            if (resolved.semester) entry.header.semester = resolved.semester;
            if (resolved.subjectName) entry.header.subjectName = resolved.subjectName;
        }
        return entry;
    }

    /** Every tracked question, in insertion order. */
    public static getAll(): QuestionEntry[] {
        return [...this.load().values()];
    }

    /** The entry for a question, or `undefined` if it has never been seen. */
    public static getEntry(subjectFile: string, questionId: string | number): QuestionEntry | undefined {
     ...
     */
    public static recordAnswer(header: HeaderInput, result: AnswerResult): QuestionEntry {
    ...
    public static isFavorite(subjectFile: string, questionId: string | number): boolean {
    ...
    public static setFavorite(header: HeaderInput, favorite: boolean): void {
        const entry = this.ensureEntry(header);
    ...
    public static toggleFavorite(header: HeaderInput): boolean {
        const entry = this.ensureEntry(header);
    ...
    public static getFavorites(): QuestionEntry[] {
    ...
    private static toModel(q: ExamQuestion): QuestionModel {
    ...
    private static loadContent(subjectFile: string): Promise<Map<string, QuestionModel>> {
        ...
        const promise = (async () => {
            const byId = new Map<string, QuestionModel>();
            try {
                const data = await Fetcher.fetchJSON(DATA_DIR + subjectFile) as ExamFile;
                for (const q of data.Questions) byId.set(String(q.Id), this.toModel(q));
            } catch (e) {
    ...
    public static async resolveModel(subjectFile: string, questionId: string | number): Promise<QuestionModel | null> {
     ...
     * are dropped.
     */
    public static async resolveFavorites(): Promise<Array<{ entry: QuestionEntry; model: QuestionModel }>> {
        const favorites = this.getFavorites();
        const files = [...new Set(favorites.map(e => e.header.subjectFile))];
        await Promise.all(files.map(f => this.loadContent(f)));

        const out: Array<{ entry: QuestionEntry; model: QuestionModel }> = [];
    ...
    private static importLegacyFavorites(): void {
        let raw: string | null;
        try {
        ...
        try {
            ...
            if (Array.isArray(legacy)) {
                for (const fav of legacy) {
                    if (!fav?.subjectFile || fav.model?.id === undefined) continue;
                    this.setFavorite({
        ...
        try { localStorage.removeItem(LEGACY_FAVORITES_KEY); } catch { /* ignore */ }
    }
}
