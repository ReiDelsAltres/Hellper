...
export default class ReIcon extends Component {
     ...
     */
    private static readonly iconLibrary: Map<string, IconConfig> = new Map([
    ...
    protected preLoad(holder: IElementHolder): Promise<void> {
        this.iconWrapper = holder.element.querySelector('.icon-wrapper') as HTMLElement;
    ...
    private updateVisuals(): void {
        ...
        this.iconWrapper?.querySelectorAll('.re-icon-svg').forEach(svg => {
            ...
            if (size) svg.classList.add(`size-${size}`);
        });
        ...
        if (this.svgElement) {
            ...
            if (flip === 'horizontal' || flip === 'both') transformParts.push('scaleX(-1)');
            if (flip === 'vertical' || flip === 'both') transformParts.push('scaleY(-1)');
        ...
        let badgeEl = this.iconWrapper?.querySelector('.re-icon-badge') as HTMLElement | null;
     ...
     * Установить размер
     */
    public setSize(size: 'xs' | 'sm' | 'md' | 'lg' | 'xl' | 'xxl'): void {
     ...
     * Установить цвет
     */
    public setColor(color: 'primary' | 'secondary' | 'tertiary' | 'additional' | 'success' | 'warning' | 'error' | 'info' | 'text' | 'empty'): void {
     ...
     * Установить вариант
     */
    public setVariant(variant: 'contrast' | 'outlined' | ''): void {
        if (variant) {
     ...
     * Установить бейдж
     */
    public setBadge(value: string | null): void {
        if (value !== null) {
